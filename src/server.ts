...
app.get('*', (req, res) => {
  // Only handle API and uploads routes, return 404 for everything else
  if (!req.url.startsWith('/api') && !req.url.startsWith('/ws') && !req.url.startsWith('/uploads')) {
    return res.status(404).json({ message: 'Not found - This is an API-only server for mobile app' });
  }
